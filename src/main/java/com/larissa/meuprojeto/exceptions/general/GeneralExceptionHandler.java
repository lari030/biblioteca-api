...
public class GeneralExceptionHandler {
  ...
  @ExceptionHandler(MethodArgumentNotValidException.class)
   ...
   List<RestErrorMessage> erros = exception.getBindingResult().getFieldErrors().stream() .map(erroCampo -> new RestErrorMessage(
   HttpStatus.BAD_REQUEST,erroCampo.getField() + ": " + erroCampo.getDefaultMessage())) .collect(Collectors.toList());
